...
def pod_count(namespace):
    pods = []
    config.load_kube_config("%s") %(kube_cfg)
    pods_list = cli.list_namespaced_pod("%s") %(namespace, watch=False)
...
def monkey(project_name):
    ...
    if not status:
        print "Test failed, looks like pods haven't been rescheduled"
        sys.exit(1)
